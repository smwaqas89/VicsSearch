# VicsSearch - Windows Installation Guide
...
Expand-Archive -Path ~\Downloads\docsearch-with-rag.zip -DestinationPath . -Force
...
tar -xf %USERPROFILE%\Downloads\docsearch-with-rag.zip
...
$env:PYTHONPATH = ".\src"
python -m docsearch.cli.main serve
```
...
```
%LOCALAPPDATA%\docsearch\docsearch.db
%LOCALAPPDATA%\docsearch\vectors\vectors.db
```

Or if using the default Unix-style path:
```
%USERPROFILE%\.local\share\docsearch\
```
...
Create config at:
```
%USERPROFILE%\.config\docsearch\config.toml
```

Or:
```
%APPDATA%\docsearch\config.toml
```
...
3. Configure:
   ...
   - Arguments: `-m docsearch.cli.main serve`
   - Environment: `PYTHONPATH=.\src`

4. Start service:
```cmd
nssm start VicsSearch
```
